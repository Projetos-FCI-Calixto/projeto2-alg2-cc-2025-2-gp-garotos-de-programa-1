...
// Função recursiva para codificar a imagem usando quadtree
char* encode(int** image, int x1, int y1, int x2, int y2) {
    // Calcular largura e altura do quadrante
    int width = x2 - x1 + 1;
    int height = y2 - y1 + 1;

    // Caso base: verificar se o quadrante é uniforme
    ...
    if (eh_uniforme) {
        ...
        // Se não uniforme, dividir em 4 quadrantes
        char* result = (char*)malloc(2 * sizeof(char));
        strcpy(result, "X");

        // Calcular divisões (altura e largura para os quadrantes)
        ...
        // Chamadas recursivas para os quadrantes, se válidos
        char* q1 = NULL;
        char* q2 = NULL;
        char* q3 = NULL;
        char* q4 = NULL;
